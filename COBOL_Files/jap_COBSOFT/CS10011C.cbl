      $set sourceformat"free"
       program-id. CS10011C.
      *>=================================================================================
      *>    
      *>                            USUARIO x PERFIL
      *>
      *>=================================================================================
       environment division.
       configuration section.
            special-names. decimal-point is comma.      

       copy CSS00800.cpy.
       copy CSS01000.cpy.
       copy CSS01100.cpy.
       copy CSS01200.cpy.
       copy CSS01300.cpy.

      *>=================================================================================
       data division.      

       copy CSF00800.cpy.
       copy CSF01000.cpy.
       copy CSF01100.cpy.
       copy CSF01200.cpy.
       copy CSF01300.cpy.
                           
      *>=================================================================================      
       working-storage section.
       
       78   c-Version                               value "a".
       78   c-ThisProgram                        value "CS10011C".
       78   c-ProgramDesc                   value "User_Yuuzaa x Profile_Purofiru".
       
       copy CSC00900.cpy.
       copy CSW00900.cpy.
       
       
       01   ws-WorkFields.
            03 ws-LineNum                         pic 9(02).
            03 ws-Line_Gyou-display                    pic x(100).

       01   ws-quadros.
            03 ws-cabecalho-Frame_Furemu_Furemu.
               05 filler                           pic x(001) value all "".
               05 filler                           pic x(114) value all "". 
               05 filler                           pic x(001) value all "".   
            03 ws-FrameLine.
               05 filler                           pic x(001) value all "".
               05 filler                           pic x(114) value all " ". 
               05 filler                           pic x(001) value all "". 
            03 ws-rodape-Frame_Furemu_Furemu.
               05 filler                           pic x(001) value all "".
               05 filler                           pic x(114) value all "". 
               05 filler                           pic x(001) value all "". 
               
       01   f-User_Yuuzaa-Profile_Purofiru.
            03 f-cpf                               pic 9(11).
            03 f-UserName                      pic x(55).
            03 f-ProfileCode                         pic 9(03).
            03 f-Name_Namae-Profile_Purofiru                       pic x(55).
            
      *>=================================================================================
       linkage section.
                 
       copy CSL00900.cpy.
      
      *>=================================================================================
       screen section.
       
       01   frm-User_Yuuzaa-Profile_Purofiru.
            03 line 11 col 21   pic x(12) value "Login (CPF):".
            03 line 11 col 34   pic x(14) from ws-mascara-cpf.
            03 line 11 col 34   pic 9(03) from f-cpf(01:03).
            03 line 11 col 38   pic 9(03) from f-cpf(04:03).
            03 line 11 col 42   pic 9(03) from f-cpf(07:03).
            03 line 11 col 46   pic 9(02) from f-cpf(10:02).
            03 line 13 col 20   pic x(13) value "Name_Namae ユーザー:".
            03 line 13 col 34   pic x(55) from f-UserName.
            03 line 15 col 19   pic x(14) value "Code プロフィール:".
            03 line 15 col 34   pic 9(03) from f-ProfileCode.
            03 line 17 col 21   pic x(12) value "Name_Namae プロフィール:".
            03 line 17 col 34   pic x(55) from f-Name_Namae-Profile_Purofiru.
            
       01   frm-Profile_Purofiru-User_Yuuzaa.
            03 line 20 col 04   pic x(116) from ws-cabecalho-Frame_Furemu_Furemu.
            03 line 21 col 04   pic x(116) from ws-FrameLine.            
            03 line 22 col 04   pic x(116) from ws-FrameLine. 
            03 line 23 col 04   pic x(116) from ws-FrameLine. 
            03 line 24 col 04   pic x(116) from ws-FrameLine. 
            03 line 25 col 04   pic x(116) from ws-FrameLine. 
            03 line 26 col 04   pic x(116) from ws-FrameLine. 
            03 line 27 col 04   pic x(116) from ws-FrameLine. 
            03 line 28 col 04   pic x(116) from ws-FrameLine. 
            03 line 29 col 04   pic x(116) from ws-FrameLine. 
            03 line 30 col 04   pic x(116) from ws-FrameLine. 
            03 line 31 col 04   pic x(116) from ws-FrameLine. 
            03 line 32 col 04   pic x(116) from ws-FrameLine. 
            03 line 33 col 04   pic x(116) from ws-FrameLine. 
            03 line 34 col 04   pic x(116) from ws-FrameLine. 
            03 line 35 col 04   pic x(116) from ws-FrameLine. 
            03 line 36 col 04   pic x(116) from ws-FrameLine. 
            03 line 37 col 04   pic x(116) from ws-FrameLine. 
            03 line 38 col 04   pic x(116) from ws-FrameLine. 
            03 line 39 col 04   pic x(116) from ws-FrameLine. 
            03 line 40 col 04   pic x(116) from ws-FrameLine.
            03 line 41 col 04   pic x(116) from ws-FrameLine.
            03 line 42 col 04   pic x(116) from ws-FrameLine.
            03 line 43 col 04   pic x(116) from ws-FrameLine.
            03 line 44 col 04   pic x(116) from ws-FrameLine.
            03 line 45 col 04   pic x(116) from ws-FrameLine.
            03 line 46 col 04   pic x(116) from ws-FrameLine.
            03 line 47 col 04   pic x(116) from ws-FrameLine.
            03 line 48 col 04   pic x(116) from ws-rodape-Frame_Furemu_Furemu.  
            
      *>=================================================================================
       procedure division using lnk-par.
       
      *>=================================================================================

       0000-Control_Seigyo section.
            perform 1000-Initialization_Shokika
            perform 2000-Processing_Shori
            perform 3000-Finalization_Shuryo.
       0000-Exit_Deguchi.    
            exit program
            stop run
       exit.
       
      *>=================================================================================
       1000-Initialization_Shokika section.
       
           initialize                             wf-FrameOptions
           
           perform 9000-Open_Akeru-i-pd00800
           perform 9000-Open_Akeru-i-pd01000
           perform 9000-Open_Akeru-i-pd01100
           perform 9000-Open_Akeru-io-pd01200
           perform 9000-Open_Akeru-io-pd01300
       
       exit.
        
      *>=================================================================================
       2000-Processing_Shori section.
       
       perform until wf-Frame_Furemu-Return_Modoru
                      
                 evaluate wf-Frame_Furemu
                      when 0
                           perform 8000-Screen_Gamen
                           perform 8000-FrameControl
                      when 9
                           perform 2999-FrameControl
                      when other   
                           move "無効なフレーム!"   to ws-Message_Messeji
                           perform 9000-Message_Messeji
                 end-evaluate
                           
            end-perform       

       exit.
       
      *>=================================================================================
       2999-FrameControl section.
        
            perform 8000-accept-Option_Opushon
            
            evaluate wf-Option_Opushon
                when 01
                     perform 2100-Add_Tsuika
                when 02
                     perform 2100-Query_Shokai
                when 03
                     perform 2100-Maintenance_Hozen
                when 04
                     perform 2100-Delete_Sakujo
                when 05
                     perform 2100-Report_Repoto
                when 99
                     set wf-Frame_Furemu-Return_Modoru         to true                       
                when other    
                     move "無効なオプション!"   to ws-Message_Messeji
                     perform 9000-Message_Messeji            
            end-evaluate
       
       exit. 
      
      *>=================================================================================
       2100-Add_Tsuika section.

            if   not lnk-AllowAdd
                 exit section
            end-if       

            perform 8000-ClearScreen       
 
            perform 9000-inclusao-Maintenance_Hozen
            
       exit.
       
      *>=================================================================================
       2100-Query_Shokai section.
       
            if   not lnk-AllowQuery
                 exit section
            end-if       

            perform 8000-ClearScreen
             
            perform until f-cpf <> zeros            
                 
                    accept f-cpf(01:03) at line 11 col 34 with update auto-skip

                    accept f-cpf(04:03) at line 11 col 38 with update auto-skip 

                    accept f-cpf(07:03) at line 11 col 42 with update auto-skip 

                    accept f-cpf(10:02) at line 11 col 46 with update auto-skip 
 
            end-perform 

            initialize                             f00800-User_Yuuzaa
            move f-cpf                             to f00800-UserID
            perform 9000-Read_Yomu-pd00800-ran
            if   not ws-OperationOK
                 string "ユーザー no Found_Mitsuketa! - [" f-cpf "]" into ws-Message_Messeji
                 perform 9000-Message_Messeji
                 perform 8000-FrameControl 
            else                
                 if   f00800-Development_Kaihatsu
                      move "ユーザー com Profile_Purofiru especial - Incluso\alterao no permitida!" to ws-Message_Messeji
                      perform 9000-Message_Messeji
                      perform 8000-FrameControl 
                      exit section                 
                 end-if 
                 
                 move f00800-UserName          to f-UserName
                 
                 perform 8000-Screen_Gamen
            end-if       
       
       exit.

      *>=================================================================================
       2100-Maintenance_Hozen section.
       
            if   not lnk-AllowMaintenance
                 exit section
            end-if       

            perform 8000-ClearScreen       
 
            perform 9000-inclusao-Maintenance_Hozen
       
       exit.

      *>=================================================================================
       2100-Delete_Sakujo section.

            if   not lnk-AllowQuery
                 exit section
            end-if       

            perform 8000-ClearScreen
             
            perform until f-cpf <> zeros            
                 
                    accept f-cpf(01:03) at line 11 col 34 with update auto-skip

                    accept f-cpf(04:03) at line 11 col 38 with update auto-skip 

                    accept f-cpf(07:03) at line 11 col 42 with update auto-skip 

                    accept f-cpf(10:02) at line 11 col 46 with update auto-skip 
 
            end-perform 

            initialize                             f00800-User_Yuuzaa
            move f-cpf                             to f00800-UserID
            perform 9000-Read_Yomu-pd00800-ran
            if   not ws-OperationOK
                 string "ユーザー no Found_Mitsuketa! - [" f-cpf "]" into ws-Message_Messeji
                 perform 9000-Message_Messeji
                 perform 8000-FrameControl 
            else                
                 if   f00800-Development_Kaihatsu
                      move "ユーザー com Profile_Purofiru especial - Excluso no permitida!" to ws-Message_Messeji
                      perform 9000-Message_Messeji
                      perform 8000-FrameControl 
                      exit section                 
                 end-if 
                 
                 move f00800-UserName          to f-UserName
                 
                 perform 8000-Screen_Gamen
                 
                 move "Excluir todos os Profile_Purofiru atrelado ao User_Yuuzaa? [S/N]" to ws-Message_Messeji
                 perform 9000-Message_Messeji
                 
                 if   ws-MessageOptionYes

                      initialize                   f01200-User_Yuuzaa-Profile_Purofiru
                      move lnk-CompanyCode          to f01200-CompanyCode
                      move lnk-BranchCode           to f01200-BranchCode
                      move f-cpf                   to f01200-UserID
                      perform 9000-str-pd01200-gtr
                      perform 9000-Read_Yomu-pd01200-nex
                      perform until not ws-OperationOK
                           or f01200-UserID <> f-cpf
                           
                           delete pd01200
                                             
                           perform 9000-Read_Yomu-pd01200-nex 
                      end-perform
                 
                      initialize                   f01300-User_Yuuzaa-modulo
                      move lnk-CompanyCode          to f01300-CompanyCode
                      move lnk-BranchCode           to f01300-BranchCode
                      move f-cpf                   to f01300-UserID
                      perform 9000-str-pd01300-gtr
                      perform 9000-Read_Yomu-pd01300-nex
                      perform until not ws-OperationOK
                           or f01300-UserID <> f-cpf
                           
                           delete pd01300
                                             
                           perform 9000-Read_Yomu-pd01300-nex 
                      end-perform

                 
                 else
                      set ws-MessageOptionYes    to true
                      
                      perform until ws-Message_Messeji-Option_Opushon-nao
                           
                           set ws-Message_Messeji-Option_Opushon-nao    to true

                           perform until exit
                           
                               accept f-ProfileCode at line 15 col 34 with update auto-skip
                               
                                initialize                        f01000-Profile_Purofiru
                                move lnk-CompanyCode               to f01000-CompanyCode
                                move lnk-BranchCode                to f01000-BranchCode
                                move f-ProfileCode                  to f01000-ProfileCode
                                perform 9000-Read_Yomu-pd01000-ran
                                if   not ws-OperationOK
                                and  not ws-RecordNotFound
                                     string "Error_Eraa na Reading_Dokusho f01000-Profile_Purofiru - " f01000-Profile_Purofiru into ws-Message_Messeji
                                     perform 9000-Message_Messeji
                                     perform 8000-FrameControl
                                     exit section
                                else
                                     if   ws-RecordNotFound
                                          move "プロフィール no cadastrado!" to ws-Message_Messeji
                                          perform 9000-Message_Messeji 
                                     else
                                          move f01000-Desc-Profile_Purofiru  to f-Name_Namae-Profile_Purofiru
                                          perform 8000-Screen_Gamen                          
                                          exit perform
                                     end-if     
                                end-if
                                                
                           end-perform
                      
                           initialize                   f01200-User_Yuuzaa-Profile_Purofiru
                           move lnk-CompanyCode          to f01200-CompanyCode
                           move lnk-BranchCode           to f01200-BranchCode
                           move f-cpf                   to f01200-UserID
                           move f-ProfileCode             to f01200-ProfileCode
                           perform 9000-Read_Yomu-pd01200-ran
                           if   not ws-OperationOK
                                exit perform cycle 
                           end-if
                      
                           initialize                   f01300-User_Yuuzaa-modulo
                           move lnk-CompanyCode          to f01300-CompanyCode
                           move lnk-BranchCode           to f01300-BranchCode
                           move f-cpf                   to f01300-UserID
                           perform 9000-str-pd01300-gtr
                           perform 9000-Read_Yomu-pd01300-nex
                           perform until not ws-OperationOK
                                or f01300-UserID <> f-cpf
                                
                                delete pd01300
                                                  
                                perform 9000-Read_Yomu-pd01300-nex 
                           end-perform

                           initialize                        f01200-User_Yuuzaa-Profile_Purofiru
                           move lnk-CompanyCode               to f01200-CompanyCode
                           move lnk-BranchCode                to f01200-BranchCode
                           move f-cpf                        to f01200-UserID
                           perform 9000-str-pd01200-gtr
                           perform 9000-Read_Yomu-pd01200-nex
                           perform until not ws-OperationOK
                                or f01200-UserID <> f-cpf
                                
                                initialize                   f01100-Profile_Purofiru-modulo
                                move lnk-CompanyCode          to f01100-CompanyCode
                                move lnk-BranchCode           to f01100-BranchCode
                                move f01200-ProfileCode        to f01100-ProfileCode
                                perform 9000-str-pd01100-gtr
                                perform 9000-Read_Yomu-pd01100-nex
                                perform until not ws-OperationOK
                                     or f01100-ProfileCode <> f01200-ProfileCode
                                     
                                     initialize                   f01300-User_Yuuzaa-modulo
                                     move lnk-CompanyCode          to f01300-CompanyCode
                                     move lnk-BranchCode           to f01300-BranchCode
                                     move f01100-ProgramCode      to f01300-ProgramCode
                                     move f01100-id-Query_Shokai      to f01300-id-Query_Shokai
                                     move f01100-id-inclusao      to f00130-id-inclusao   
                                     move f01100-id-Maintenance_Hozen    to f01300-id-Maintenance_Hozen
                                     move f01100-id-Delete_Sakujo      to f00130-id-Delete_Sakujo
                                     perform 9000-Write_Kaku-pd01300
                                     if   not ws-OperationOK
                                          perform 9000-Rewrite_Kakinaosu-pd01300
                                          if   not ws-OperationOK
                                               string "Error_Eraa ao Write_Kaku f01300-User_Yuuzaa-modulo - " ws-AccessResult into ws-Message_Messeji
                                               perform 9000-Message_Messeji
                                          end-if
                                     end-if
                                                                
                                     perform 9000-Read_Yomu-pd01100-nex 
                                end-perform
                                
                               perform 9000-Read_Yomu-pd01200-nex
                           end-perform 
                           
                      end-perform
                 
                 end-if 
                                  
            end-if        
       
       exit.       
       
      *>=================================================================================
       2100-Report_Repoto section.
       
       
       exit.        
                   
      *>=================================================================================
       3000-Finalization_Shuryo section.
       
            close pd00800
            close pd01000
       
       
       exit.                                                   
      *>=================================================================================
      *> Rotinas Genrias - Frame
 
       copy CSC00903.cpy. *> Frame_Furemu

      *>=================================================================================       
       8000-Screen_Gamen section.
       
            perform 9000-StandardFrame
            display frm-User_Yuuzaa-Profile_Purofiru
            
            perform 9000-Screen_Gamen-Profile_Purofiru-User_Yuuzaa            
       
       exit.          

      *>=================================================================================       
       8000-ClearScreen section.
       
            initialize                             f-User_Yuuzaa-Profile_Purofiru
            
            perform 9000-StandardFrame
            display frm-User_Yuuzaa-Profile_Purofiru
            
            perform 9000-Screen_Gamen-Profile_Purofiru-User_Yuuzaa            
       
       exit.                                
      *>=================================================================================       
                               
      *>=================================================================================
      *> Rotinas Genrias
      

       copy CSP00900.cpy. *> Padro

      *>=================================================================================                          
       9000-Screen_Gamen-Profile_Purofiru-User_Yuuzaa section.
       
            display frm-Profile_Purofiru-User_Yuuzaa
            
            if   f-cpf not greater zeros
                 exit section
            end-if
            
            move 21                                to ws-LineNum
            
            initialize                             f01200-User_Yuuzaa-Profile_Purofiru
            move lnk-CompanyCode                    to f01200-CompanyCode
            move lnk-BranchCode                     to f01200-BranchCode
            move f-cpf                             to f01200-UserID
            perform 9000-str-pd01200-gtr
            perform 9000-Read_Yomu-pd01200-nex
            perform until not ws-OperationOK
                 or f01200-UserID <> f-cpf
                 
                 initialize                        f01000-Profile_Purofiru
                 move lnk-CompanyCode               to f01000-CompanyCode
                 move lnk-BranchCode                to f01000-BranchCode
                 move f01200-ProfileCode             to f01000-ProfileCode
                 perform 9000-Read_Yomu-pd01000-ran
                 if   not ws-OperationOK
                      exit perform cycle
                 end-if      
                 
                 move spaces                       to ws-Line_Gyou-display
                 
                 string f01000-ProfileCode " - " f01000-Desc-Profile_Purofiru delimited by "  " into ws-Line_Gyou-display

                 display ws-Line_Gyou-display at line ws-LineNum col 06 
                 
                 add 01                            to ws-LineNum
                 
                 if   ws-LineNum > 47
                      exit perform
                 end-if                   
                 
                 perform 9000-Read_Yomu-pd01200-nex      
            end-perform     
       
       exit.                   
      *>=================================================================================
       9000-inclusao-Maintenance_Hozen section.

            perform until f-cpf <> zeros            
                 
                    accept f-cpf(01:03) at line 11 col 34 with update auto-skip

                    accept f-cpf(04:03) at line 11 col 38 with update auto-skip 

                    accept f-cpf(07:03) at line 11 col 42 with update auto-skip 

                    accept f-cpf(10:02) at line 11 col 46 with update auto-skip 
 
            end-perform 

            initialize                             f00800-User_Yuuzaa
            move f-cpf                             to f00800-UserID
            perform 9000-Read_Yomu-pd00800-ran
            if   not ws-OperationOK
                 string "ユーザー no Found_Mitsuketa! - [" f-cpf "]" into ws-Message_Messeji
                 perform 9000-Message_Messeji
                 perform 8000-FrameControl 
            else                
                 if   f00800-Development_Kaihatsu
                      move "ユーザー com Profile_Purofiru especial - Incluso\alterao no permitida!" to ws-Message_Messeji
                      perform 9000-Message_Messeji
                      perform 8000-FrameControl 
                      exit section                 
                 end-if 
                 
                 move f00800-UserName          to f-UserName
                 
                 perform 8000-Screen_Gamen
            end-if
            
            perform until exit
            
                accept f-ProfileCode at line 15 col 34 with update auto-skip
                
                 initialize                        f01000-Profile_Purofiru
                 move lnk-CompanyCode               to f01000-CompanyCode
                 move lnk-BranchCode                to f01000-BranchCode
                 move f-ProfileCode                  to f01000-ProfileCode
                 perform 9000-Read_Yomu-pd01000-ran
                 if   not ws-OperationOK
                 and  not ws-RecordNotFound
                      string "Error_Eraa na Reading_Dokusho f01000-Profile_Purofiru - " f01000-Profile_Purofiru into ws-Message_Messeji
                      perform 9000-Message_Messeji
                      perform 8000-FrameControl
                      exit section
                 else
                      if   ws-RecordNotFound
                           move "プロフィール no cadastrado!" to ws-Message_Messeji
                           perform 9000-Message_Messeji 
                      else
                           move f01000-Desc-Profile_Purofiru  to f-Name_Namae-Profile_Purofiru
                           perform 8000-Screen_Gamen                          
                           exit perform
                      end-if     
                 end-if
                                 
            end-perform
            
            move "Confirm_Kakunin incluso\alterao do Record_Kiroku? [S/N]"   to ws-Message_Messeji
            perform 9000-Message_Messeji
            
            if   ws-MessageOptionYes
                 
                 initialize                        f01200-User_Yuuzaa-Profile_Purofiru
                 move lnk-CompanyCode               to f01200-CompanyCode
                 move lnk-BranchCode                to f01200-BranchCode
                 move f-cpf                        to f01200-UserID
                 move f-ProfileCode                  to f01200-ProfileCode
                 perform 9000-Write_Kaku-pd01200
                 if   not ws-OperationOK
                      perform 9000-Rewrite_Kakinaosu-pd01200
                      if   not ws-OperationOK
                           string "Error_Eraa ao Write_Kaku f01200-User_Yuuzaa-Profile_Purofiru - " ws-AccessResult into ws-Message_Messeji
                           perform 9000-Message_Messeji
                           exit section
                      end-if
                 end-if
                 
                 *> Exclui todos os programas alterados ao User_Yuuzaa.
                 
                 initialize                        f01300-User_Yuuzaa-modulo
                 move lnk-CompanyCode               to f01300-CompanyCode
                 move lnk-BranchCode                to f01300-BranchCode
                 move f-cpf                        to f01300-UserID
                 perform 9000-str-pd01300-gtr
                 perform 9000-Read_Yomu-pd01300-nex
                 perform until not ws-OperationOK
                      or f01300-UserID <> f-cpf
                      
                      delete pd01300
                                        
                      perform 9000-Read_Yomu-pd01300-nex 
                 end-perform
                 
                 initialize                        f01200-User_Yuuzaa-Profile_Purofiru
                 move lnk-CompanyCode               to f01200-CompanyCode
                 move lnk-BranchCode                to f01200-BranchCode
                 move f-cpf                        to f01200-UserID
                 perform 9000-str-pd01200-gtr
                 perform 9000-Read_Yomu-pd01200-nex
                 perform until not ws-OperationOK
                      or f01200-UserID <> f-cpf
                      
                      initialize                   f01100-Profile_Purofiru-modulo
                      move lnk-CompanyCode          to f01100-CompanyCode
                      move lnk-BranchCode           to f01100-BranchCode
                      move f01200-ProfileCode        to f01100-ProfileCode
                      perform 9000-str-pd01100-gtr
                      perform 9000-Read_Yomu-pd01100-nex
                      perform until not ws-OperationOK
                           or f01100-ProfileCode <> f01200-ProfileCode
                           
                           initialize                   f01300-User_Yuuzaa-modulo
                           move lnk-CompanyCode          to f01300-CompanyCode
                           move lnk-BranchCode           to f01300-BranchCode
                           move f01100-ProgramCode      to f01300-ProgramCode
                           move f01100-id-Query_Shokai      to f01300-id-Query_Shokai
                           move f01100-id-inclusao      to f00130-id-inclusao   
                           move f01100-id-Maintenance_Hozen    to f01300-id-Maintenance_Hozen
                           move f01100-id-Delete_Sakujo      to f00130-id-Delete_Sakujo
                           perform 9000-Write_Kaku-pd01300
                           if   not ws-OperationOK
                                perform 9000-Rewrite_Kakinaosu-pd01300
                                if   not ws-OperationOK
                                     string "Error_Eraa ao Write_Kaku f01300-User_Yuuzaa-modulo - " ws-AccessResult into ws-Message_Messeji
                                     perform 9000-Message_Messeji
                                end-if
                           end-if
                                                      
                           perform 9000-Read_Yomu-pd01100-nex 
                      end-perform
                      
                     perform 9000-Read_Yomu-pd01200-nex
                 end-perform 
                 
                 perform 8000-Screen_Gamen
                 
            end-if
       
       exit.
              
      *>=================================================================================
      *> 読み取り
      
       copy CSR00800.cpy.
       copy CSR01000.cpy.
       copy CSR01100.cpy.
       copy CSR01200.cpy.
       copy CSR01300.cpy.            